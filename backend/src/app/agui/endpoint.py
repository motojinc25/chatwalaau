...
async def _stream_with_reasoning(
    ...
    try:
        ...
        # SECOND GATE (PRP-0185 step 2, UDR-0167 D17). The run-target's declaration is
        # only one of two conditions; the model's offering is the other. An offering
        # that declares `native_structured_output: false` makes
        # `structured_output_support(...)["supported"]` false, and since PRP-0131
        # removed the forced-tool-use fallback (UDR-0058 D10) `build_structured_output`
        # then returns `{}` -- nothing constrains the answer.
        #
        # Without this gate the two sides disagreed and the WRONG one was believed: the
        # marker event still fired, so the SPA rendered a plain prose answer inside a
        # JSON code fence under a "Structured output" badge, while no format had been
        # sent at all. Claiming a guarantee the request never made is worse than not
        # offering the feature -- an operator can work around a missing capability and
        # cannot work around a false one. It is the failure UDR-0102 exists to prevent,
        # one layer up, and it is the two-gate shape image generation already carries
        # (an image OFFERING must exist AND the capability must not be withheld).
        #
        # Resolved to OFF, never to an error (UDR-0058 D2's posture): a withheld
        # capability is a deployment fact, not a request defect. DEMO_MODE resolves no
        # structured shape at all (UDR-0045 D7/D10) and its marker is already
        # suppressed downstream, so it is not probed here.
        if structured_active and not is_demo_mode():
            try:
                supported = bool(providers.structured_output_support(effective_model).get("supported", True))
            except Exception:
                # Best-effort, like every other capability probe: a lookup that cannot
                # be made must not withdraw a feature the operator did configure.
                logger.debug("Structured-output probe failed for %s", effective_model, exc_info=True)
                supported = True
            if not supported:
                logger.info(
                    "Structured output is configured on %s but model '%s' withholds "
                    "native_structured_output; resolved OFF for this turn.",
                    getattr(spec, "id", "?"),
                    effective_model,
                )
