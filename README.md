...
- Auto chat titles (optional): set `SESSION_TITLE_MODE=llm` and a new chat is named by a short LLM summary of your opening exchange instead of the first message's leading text. The chat appears in the sidebar the instant you send (with a small spinner while the title is generated) and updates in **real time** when it is ready -- pushed over a WebSocket, no reload. Pick a small/cheap model with `SESSION_TITLE_MODEL` (empty = your session's model). Generation runs in the background and never blocks or delays your message; on any failure the simple truncated title remains. Default `truncate` keeps the previous behavior exactly
