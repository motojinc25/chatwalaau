...
- **Survives a restart** -- deploying or restarting the backend no longer signs anyone out, and a message you were typing is never thrown away: if the server is unreachable your text comes back to the input box with a **Retry** button, and an expired sign-in opens a dialog **over your intact chat** instead of reloading the page. Sessions persist as SHA-256 digests only -- never the token itself (`AUTH_SESSION_PERSIST`, on by default)
