...
- **Token accounting** (v0.145.0) -- an append-only ledger of what every lane spent (chats, harness agents, Teams, the OpenAI-compatible API, and the background passes you never see), read back with `GET /api/usage/summary` by day, month, chat, model or lane. Interrupted turns are recorded too; deleting a chat no longer deletes its spending record. Token counts only -- no prices, which change independently and would go stale. Workflow runs are **not** counted, and every response says so -- these are what the observable work consumed, not what you were charged. As of **v0.145.1** that statement is corrected: it also listed the framework's context-compaction calls, which no configured strategy makes, so the ledger was being reported as less complete than it is. Harness agent runs are now verified end to end rather than assumed correct -- and the measurement behind that is worth knowing: a harness agent works in a loop, and asking the framework for a run's "final response" returns only the last iteration (131 of 420 output tokens on a three-iteration turn), which is why the ledger totals every model call as it happens. [Docs](https://www.chatwalaau.com/docs/api-and-cli/usage-statistics)
- **App Settings, in the app** -- as of **v0.129.0** the runtime tuning knobs no longer live in `.env`. **52 settings** -- generation, chat & session, memory, speech, RAG, limits and the cron schedule -- moved into an **App Settings** screen (the renamed Model Settings modal, now hosting the Model Offering Catalog *and* seven settings groups), backed by an operator-owned `app_settings.jsonc`. The first-run `.env` shrinks from ~150 keys to ~97, leaving only endpoints, credentials, ports, storage locations and feature gates. Every control **says when it takes effect** -- *applies immediately* (40 settings), *rebuilds agents* in place (8), or *restart required* (12) -- so a value that is saved but not yet live never looks like it took effect; ChatWalaʻau never restarts itself, it tells you which keys are waiting on you. Upgrading is one command (`chatwalaau settings migrate --write`) and the startup log **names** every relocated key still sitting in your `.env`. Secrets, bootstrap paths and every feature **enable-gate** deliberately stay in `.env` -- a screen that could widen the sandbox would hand that power to whoever reaches the screen, so `CODING_ENABLED` stays put while `CODING_BASH_TIMEOUT` moves. A settings file written by a **newer** release will not stop an older one from starting: keys it does not recognise are kept, not rejected. As of **v0.148.0** the *rebuilds agents* badge is finally accurate for the three history-compaction settings: the rebuild had re-used the compaction strategy built when the process started, so a save reported success while every running agent kept the previous value until the backend was restarted -- changing the kept-groups window from 10 to 4 mid-session did nothing. Saving now applies to the next turn everywhere the main agents serve (DevUI, which builds its agent once per thread, still needs a restart)
...
- **API & CLI:** [OpenAI-compatible API](https://www.chatwalaau.com/docs/api-and-cli/openai-compatible-api) · [Authentication](https://www.chatwalaau.com/docs/api-and-cli/authentication) · [CLI](https://www.chatwalaau.com/docs/api-and-cli/cli) · [Usage Statistics](https://www.chatwalaau.com/docs/api-and-cli/usage-statistics)
