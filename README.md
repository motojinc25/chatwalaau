...
- **Declarative agents** -- define an agent (persona, model, per-agent tools, output policy) in a YAML file or a built-in GUI editor, and switch the active agent at runtime from the web app; the built-in CORE agent reproduces the default behavior (opt-in custom agents via `DECLARATIVE_AGENTS_DIR`)
